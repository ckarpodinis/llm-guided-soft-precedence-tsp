...
for c in raw_constraints:
    ...
    #print('{}->{}'.format(ordered_nodes[:-1], ordered_nodes[1:]))
...
u = {
    i: m.addVar(lb=1, ub=len(N), vtype=GRB.CONTINUOUS, name=f"u_{i}")
    for i in N
}
...
for (i,j), var in s.items():
    if i in pos0 and j in pos0:
        var.start = max(0, pos0[i] + 1 - pos0[j])
...
print("\nSlack variables (violated cluster orders):")
for (i, j), var in s.items():
    if var.X > 1e-6:
        print(f"s[{i} -> {j}] = {var.X:.3f}")
